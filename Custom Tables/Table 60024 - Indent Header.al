table 60024 "Indent Header"
{
    ...
    fields
    {
        ...
        field(8;"Delivery Location";Code[20])
        {
            TableRelation = IF ("Delivery Place"=CONST(Store)) Location WHERE (Code=FILTER(<>'CS STR'&<>'R&D STR'))
                            ELSE IF ("Delivery Place"=CONST(Customer)) Customer;
        }
    ...
    begin
        ...
        if not (UserId in ['EFFTRONICS\VISHNUPRIYA','EFFTRONICS\ANILKUMAR','EFFTRONICS\ANVESH','EFFTRONICS\SPURTHI','EFFTRONICS\VIJAYA','EFFTRONICS\SUJANI','EFFTRONICS\RRAHUL']) then
        begin
          UserGRec.Reset;
    ...
    begin
        ...
        if IndentLine.Find('-') then
          repeat
            if /*(IndentLine.Type = IndentLine.Type :: Description) AND*/ (IndentLine.Type = IndentLine.Type :: Item)  then
            begin
    ...
    begin
        ...
        if IndentLine.Find('-') then
          repeat
            if IndentLine.Type <> IndentLine.Type :: Description then
            begin
    ...
    begin
        ...
        if IndentLine.Find('-') then
        repeat
          if /*(IndentLine.Type = IndentLine.Type :: Description) OR*/ (IndentLine.Type = IndentLine.Type::Item) then
          begin
            IndentLine.TestField(IndentLine."No.");
            IndentLine.TestField(IndentLine.Quantity);
            IndentLine.TestField(IndentLine."ICN No."); //added By santhosh Kumar.korada
